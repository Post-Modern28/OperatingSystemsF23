...
int main(int argc, char *argv[]) {
    ...
    while (command != NULL) {
        ...
        //this cycle is needed for aging algorithm only
        for (int i = 0; i < PAGE_SIZE; i++){
        	page_table[i].count >> 1;
        }
        page_table[page_num].reference_count++;
        page_table[page_num].count = page_table[page_num].count | (ONE << 8*sizeof(unsigned long long)-1);
 		if (operation_type == 'W'){
 	...
 	close(fd);
 	printf("Hit rate is %.3f\n(%.0f hits out of %.0f)\n", hits/overall, hits, overall);
